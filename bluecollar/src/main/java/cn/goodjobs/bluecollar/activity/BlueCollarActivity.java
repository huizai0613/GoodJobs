...
public class BlueCollarActivity extends BaseActivity
{
    ...
    public void setCurSelectJobCate(String curSelectJobCate)
    {
        ((BlueJobFragment) fragmentList.get(1)).setSelectCate(curSelectJobCate);
    }
    ...
    protected void onNewIntent(Intent intent)
    {
        ...
        if (pageIndex == 0) {
            onClick(btnFooter1);
            ((BlueHomeFragment) fragmentList.get(0)).getDataFromServer();
        } else if (pageIndex == 3) {
            onClick(btnFooter4);
            ((BlueInfoCenterFragment) fragmentList.get(3)).getDataFromServer();
        }
    }
